...
- **Shannon Entropy Calculation**: Monitoramento do nível de previsibilidade/ruído da sequência estocástica do preço. Retorna grau absoluto de incerteza da série.
- **Regime Detection**: Identifica se estamos rodando num regime Trending, Choppy, Liquidation Cascade, Squeeze. E baseia a agressão do bot nestes status.
...
- **Dynamic Lot Sizing (Kelly Compound)**: Aloca o sizing progressivo escalando como juros compostos a cada reavaliação quântica do Risco.
- **Circuit Breakers Dinâmicos**: Corta a exposição caso exceda limite de drawdowns.
- **HFT Throttling**: Cap de 5 execuções por candle M1 no executor sniper.
...
### 2B. SIMULAÇÃO QUANTUM MONTE CARLO (Phase 8)
