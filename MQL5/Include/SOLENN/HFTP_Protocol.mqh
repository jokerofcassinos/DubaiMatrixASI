...
enum ENUM_MSGPACK_TYPE {
    TYPE_INT      = 0xCF, // uint64
    TYPE_DOUBLE   = 0xCB, // float64
    TYPE_STRING   = 0xDB, // string32
    TYPE_MAP      = 0xDF, // map32
    TYPE_TRUE     = 0xC3,
    TYPE_FALSE    = 0xC2,
    TYPE_NULL     = 0xC0
...
public:
    ...
    void PackInt(long val) {
        ...
        buffer[offset++] = (uchar)TYPE_INT;
        for(int i=7; i>=0; i--) buffer[offset++] = (uchar)(val >> (i*8));
    }

    void PackDouble(double val) {
        ArrayResize(buffer, offset + 9);
        buffer[offset++] = (uchar)TYPE_DOUBLE;
        uchar bytes[8];
        StructToCharArray(val, bytes);
        for(int i=7; i>=0; i--) buffer[offset++] = bytes[i]; // Reverse for BE
    }
    ...
    void PackString(string val) {
        ...
        buffer[offset++] = (uchar)TYPE_STRING;
        // 4 bytes length (BE)
