...
def get_status():
    ...
    try:
        ...
        if "not a git repository" in e.stderr.lower():
            ...
            run_cmd("git init")
            # Adicionar branch solenn como default [Ω-Sovereignty]
            run_cmd("git checkout -b solenn")
            return "NOVO_REPO"
        else:
...
def github_sync():
    ...
    # 1. Adicionar todos os arquivos (O sistema respeitará o .gitignore [Ω])
    print("🔄 Adicionando artefatos ao index do Git (Respeitando .gitignore)...")
    run_cmd("git add .")
    ...
    # Garantir que estamos na branch solenn (Ω-Evolution)
    try:
        branch = "solenn"
        current_branch = run_cmd("git rev-parse --abbrev-ref HEAD")
        if current_branch != branch:
            print(f"🔄 Transmutando branch de {current_branch} para {branch}...")
