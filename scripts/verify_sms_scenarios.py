...
def run_scenario(trinity, asi_state, indicators, name, curvature, velocity, coherence, phi, expected_unlocked=True):
    ...
    # [FIX] Use correct tick structure and let property calculate price
    mid_price = 70000.0
    ...
    # [FIX] Explicitly set raw_signal to float to avoid MagicMock comparison errors
    q_state.raw_signal = 0.35 
    ...
    # [FIX] Bypass startup cooldown by mocking both timestamps
    trinity._startup_timestamp = time.time() - 1000 
    trinity._creation_time = time.time() - 1000 

    # [FIX] Correct argument order: quantum_state, regime_state, snapshot, asi_state
    ...
    if decision:
        ...
        print("❌ FAILURE: Decision returned None (Crash inside decide?)")

if __name__ == "__main__":
    print("🔬 DUBAI MATRIX ASI — SMS SOVEREIGNTY VERIFICATION")
    print("==================================================")
    ...
    run_scenario(trinity, asi_state, indicators, 
                 ...
                 curvature=0.09, velocity=12.0, coherence=0.40, phi=0.01, expected_unlocked=True)
    ...
    run_scenario(trinity, asi_state, indicators, 
                 ...
                 curvature=0.20, velocity=3.0, coherence=0.45, phi=0.01, expected_unlocked=True)

    # CASO 3: Sem Cobertura SMS (Baixa Curvatura, Baixa Velocidade)
    # Deve ser vetado pelo filtro de Phi/Coerência padrão
    run_scenario(trinity, asi_state, indicators, 
                 "SCENARIO 3: NO SMS COVERAGE (Should Veto)", 
